...
**Input context**: Project uses Unity 6 LTS with C# (configured in technical-preferences.md).
**Input**: "Prototype a basic grid movement system — player clicks a tile and the character moves to it."
**Expected behavior**:
- Produces the prototype in C# — not Python, Lua, or pseudocode
- Uses Unity types appropriate for a grid: a grid manager or 2D tilemap system, and a CharacterController/Transform-based player
