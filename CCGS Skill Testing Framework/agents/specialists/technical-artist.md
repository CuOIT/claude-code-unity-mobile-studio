...
**Expected behavior:**
- Produces shader code or a Shader Graph node spec appropriate to the configured engine (Unity Shader Graph / HLSL URP Shader — the only supported render path of this edition)
...
**Expected behavior:**
- Checks the engine version reference (e.g., `docs/engine-reference/unity/VERSION.md`) before suggesting any API
- Flags if the requested API is post-cutoff (e.g., Unity 6 rendering API changes)
